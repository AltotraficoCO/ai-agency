/**
 * Cómo le pregunta algo el agente de Marketing al cliente.
 *
 * Mismas dos formas que en el Webmaster y por la misma razón: un agente que
 * cierra su tarea con «¿te parece bien?» deja al cliente sin forma de
 * contestar. `pedir_aprobacion` es un sí o un no sobre una propuesta;
...
export const pedirAprobacion = defineTool({
  ...
  inputSchema: z.object({
    propuesta: z
      ...
      .min(10)
      .max(600)
      .describe("Lo que harás si aprueba, con el dinero que implica, en una o dos frases."),
  }),
  sensitive: true,
  creditCost: 0,
  scopes: [],
...
export const preguntarAlCliente = defineTool({
  ...
  whenToUse:
    "cuando el encargo sea ambiguo y necesites que el cliente elija o precise algo: qué cuenta, qué campaña, qué objetivo",
  inputSchema: z.object({
  ...
  async execute(ctx, input): Promise<Record<string, unknown>> {
    const { cuentas, workspaceId } = entorno(ctx, "preguntar_al_cliente");
    const huella = huellaAccion(cuentas.taskId, "preguntar_al_cliente", input);
    const previa = await cuentas.approvals.check({ workspaceId, taskId: cuentas.taskId, huella });
    if (previa) {
      return {
        ya_respondida: true,
    ...
    const solicitud = await cuentas.approvals.request({
      workspaceId,
