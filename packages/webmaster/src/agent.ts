...
- LISTADOS DE ENTRADAS mal pintados (las entradas salen sin título, no se puede pinchar en ellas, o no pegan con el resto del sitio): NO se arreglan rehaciendo la página, porque rehacerla se lleva por delante el listado. Léela con wp_leer_diseno_pagina y mira los ajustes que te devuelve cada widget: un widget de bucle o de archivo (loop-grid, archive-posts, posts) enseña el título con show_title, enlaza al artículo con link_to y se viste con title_color, text_color y typography_*. Cámbialos con wp_editar_diseno_pagina y la acción cambiar_ajustes. Si el título directamente NO está entre los widgets, añádelo con la acción anadir_widget y tipo theme-post-title.
- Si el listado pinta cada entrada con una PLANTILLA de bucle aparte (la verás en wp_listar_plantillas_elementor como loop-item o similar), el título y el enlace viven ahí y no en la página: edítala con wp_editar_plantilla_elementor, con las mismas dos acciones.
