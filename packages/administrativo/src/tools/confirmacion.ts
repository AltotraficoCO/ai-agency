/**
 * Cómo le pregunta algo el agente Administrativo al cliente.
 *
...
export const pedirAprobacion = defineTool({
  ...
  inputSchema: z.object({
    propuesta: z
      ...
      .min(10)
      .max(600)
      .describe("Lo que harás si aprueba, con el importe y el cliente, en una o dos frases."),
  }),
  sensitive: true,
  creditCost: 0,
  scopes: [],
...
export const preguntarAlCliente = defineTool({
  ...
  whenToUse:
    "cuando el encargo sea ambiguo y necesites que el cliente elija o precise algo: a quién facturar, por cuánto, con qué impuesto",
  inputSchema: z.object({
  ...
  async execute(ctx, input): Promise<Record<string, unknown>> {
    const { libros, workspaceId } = entorno(ctx, "preguntar_al_cliente");
    const huella = huellaAccion(libros.taskId, "preguntar_al_cliente", input);
    const previa = await libros.approvals.check({ workspaceId, taskId: libros.taskId, huella });
    if (previa) {
      return {
        ya_respondida: true,
    ...
    const solicitud = await libros.approvals.request({
      workspaceId,
